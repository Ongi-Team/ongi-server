...
# 3. MQTT 실행 여부 확인
if ! docker ps | grep -q ongi-mqtt; then
...
EOF
  ...
  docker run -d \
    ...
    --restart unless-stopped \
    -p 1883:1883 \
    -v /home/ubuntu/mosquitto/config/mosquitto.conf:/mosquitto/config/mosquitto.conf \
    eclipse-mosquitto:2
fi

# 4. 현재 nginx가 바라보는 포트 확인
